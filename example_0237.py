...
Example stimulus 237
created by Eline Van Geert

Wallpaper patterns: P3M1
"""
...
stimname = "example_0237"

# Define template parameters
baseshape = Image("svg/example_0223.svg")
basesize = (200,(4/6)*200*np.sqrt(3))
basewidth = basesize[0]
baseheight = basesize[1]

# Create template tile
stimulus = Grid(n_rows = 2, n_cols = 3, background_color = "none",
                row_spacing = int((1/3)*200*np.sqrt(3)), col_spacing = 100, 
                x_margin = 0, y_margin = 0)
stimulus.positions = Positions.CreateCustomPositions(x = [0, 100, 200, 0,100,200], 
                                                     y = [0,-int((1/6)*200*np.sqrt(3)),0, int((1/3)*200*np.sqrt(3)),int((1/2)*200*np.sqrt(3)),int((1/3)*200*np.sqrt(3))])
...
stimulus.orientations = GridPattern.RepeatAcrossElements([0,0,120,120,240,240])
...
structuresize = (400,(4/6)*400*np.sqrt(3))
